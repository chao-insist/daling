$(function(){
	...
	$('.add,.imme').on('click',function(){
		...
		$.cookie('good',{img:img,title:title1,info:title2,price:price,num:num},{expires:30,path:'/'});
...
$(function(){
	...
	if($.cookie('good') != null){		//判断是否存在名为good的cookie
		...
		var str = '<div class="cart-list"><ul><li class="td-check"><input type="checkbox" class="check" name="good"></li><li class="td-good"><a class="cover" href="javascript:;" style="background-image: url('+$.cookie('good').img+')"></a><a href="javascript:;" class="name"><span class="title">['+$.cookie('good').title+']</span><br><span>'+$.cookie('good').info+'</span></a></li><li class="td-price">￥<span class="price">'+$.cookie('good').price+'</span></li><li class="td-number"><a href="javascript:;" class="btn-reduce"><span class="ico-reduce"></span></a><input type="text" value="'+$.cookie('good').num+'" class="num-text"><a href="javascript:;" class="btn-add f1"><span class="ico-add"></span></a></li><li class="td-total">￥<span class="total"></span></li><li class="td-handle"><a href="javascript:;" class="detach">删除</a></li></ul></div>'
